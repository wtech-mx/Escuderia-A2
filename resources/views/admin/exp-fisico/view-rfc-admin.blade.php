...
<div class="row bg-down-blue " style="border-radius: 0 0 0 0; height: 95vh;">
                    ...
                    @if ($exp_rfc->count())
                        @foreach($exp_rfc as $item)
                            @php
                                $texto= substr($item->rfc, -3);
                            @endphp
                            <div class="col-6">
                                 <a  class="" data-toggle="modal" data-target="#modal-doc-{{$item->id}}">
                                        @if($texto == 'pdf')
                                            <p class="text-center">
                                                <iframe width="140" height="140" src="{{asset('exp-rfc/'.$item->rfc)}}" frameborder="0"></iframe>
                               ...
                               <form method="POST" action="{{route('store_admin.view-rfc-admin', $automovil->id)}}" enctype="multipart/form-data" role="form">
                                ...
                                <div class="col-12 mt-3">
                                    <div class=" custom-file mb-3">
                                        <input type="file" class="custom-file-input input-group-text image" id="rfc" name="rfc">
...
<script>
    $('#rfc').ijaboCropTool({
        ...
        processUrl: '{{ route('store_admin.view-rfc-admin', $automovil->id) }}',
