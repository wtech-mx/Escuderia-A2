...
<div class="row bg-down-blue " style="border-radius: 0 0 0 0; height: 95vh;">
                        ...
                        <script>
                            Swal.fire({
                              title: 'Exito!!',
                              html:
                                'Se ha agragado la <b>INE</b>, ' +
                                'Exitosamente',
                              // text: 'Se ha agragado la "MARCA" Exitosamente',
                              imageUrl: '{{ asset('img/icon/color/tarjeta-de-identificacion.png') }}',
                              background: '#fff',
                              imageWidth: 150,
                              imageHeight: 150,
                              imageAlt: 'tarjeta-de-identificacion IMG',
                            })
                        </script>
                        @endif
                    ...
                    @if ($exp_ine->count())
                        @foreach($exp_ine as $item)
                            @php
                                $texto= substr($item->ine, -3);
                            @endphp
                            <div class="col-6">
                                <a type="button" class="" data-toggle="modal" data-target="#modal-doc-{{$item->id}}">
                                        @if($texto == 'pdf')
                                            <p class="text-center">
                                                <iframe width="140" height="140" src="{{asset('exp-ine/'.$item->ine)}}" frameborder="0"></iframe>
                                                <p class="text-center text-white">{{$item->titulo}}</p>
                                            </p>
                                        @else
                                            <p class="text-center">
                                                    <img class="d-inline mb-2" src="{{asset('exp-ine/'.$item->ine)}}" alt="{{$item->ine}}" width="100px">
                                                <p class="text-center text-white">{{$item->titulo}}</p>
                                            </p>
                                        @endif
                                </a>
                                  ...
                                  <div class="modal-body">
                                      ...
                                      <div class="row justify-content-center">
                                          <div class="d-flex align-items-center">
                                              <div class="col-11">
                                                  <p class="text-center">
                                                      <img class="" src="{{asset('exp-ine/'.$item->ine)}}" alt="{{$item->ine}}" style="height: 300px!important;">
                                                  </p>
                                              </div>
                                              <div class="col-1">
                                                    <a type="button" class="btn btn-danger text-white p-2 mt-5 mb-5" data-toggle="modal" data-target="#modaline{{$item->id}}">
                          ...
                          <div class="modal-dialog modal-sm modal-dialog-centered">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                               ...
                               <form method="POST" action="{{route('store_admin.view-ine-admin', $automovil->id)}}" enctype="multipart/form-data" role="form">
                                         @csrf
