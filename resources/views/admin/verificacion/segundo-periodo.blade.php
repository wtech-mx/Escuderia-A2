                    ...
                    <form class="p-5" method="POST"
                        action="{{ route('update_periodo2.verificacion', $verificacion_segunda->id) }}"
                        enctype="multipart/form-data" role="form">
                        @csrf

                        <div class="col-12 mt-1">
                            <div class="input-group form-group">
                                <input type="hidden" class="form-control" id='id_sector' name="id_sector" value="{{ $verificacion_segunda->id_sector }}">
                            </div>
                            ...
                            @if ($verificacion->id_user == null)
                                <p class="text-center mb-5"
                                    style="color: #00d62e; font: normal normal bold 20px/27px Segoe UI;">
                                    <strong>{{ $verificacion->UserEmpresa->name }} /
                                        {{ $verificacion->Automovil->placas }}</strong></p>

                            @else
                                <p class="text-center mb-5"
                                    style="color: #00d62e; font: normal normal bold 20px/27px Segoe UI;">
                                    <strong>{{ $verificacion->User->name }} /
                                        {{ $verificacion->Automovil->placas }}</strong></p>
                            @endif
                            ...
                            <div class="input-group col-6">
                                <input type="text" class="form-control"
                                    placeholder="{{ $verificacion->Automovil->placas }}" readonly>
                            </div>
                            ...
                            <div class="input-group form-group">
                                ...
                                <input type="date" class="form-control" placeholder="MM/DD/YYY"
                                    style="border-radius: 0  10px 10px 0;" id='segundo_semestre' name="segundo_semestre"
                                    value="{{ $verificacion_segunda->segundo_semestre }}">
                            </div>
